...
/*   Updated: 2019/09/08 17:46:09 by wmaykit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_add_lim_precision_num_to_buf(t_real *n, t_print *sp, int digit)
{
	unsigned	*start;
	unsigned	*end;
	unsigned	*b;
	...
	if ((tmp = digit - sp->format.precision) <= 0)
	{
		ft_add_big_num_to_buf(start, n->fractional, sp);
		sp->format.len.zero = sp->format.precision - digit;
	}
	else
	{
...
static void ft_add_fractional_to_buf(t_real *num, t_print *sp)
{
	...
	sign_digit = 0;
/*	while (!num->fractional[num->frac_position] && num->frac_position > -1)
		--num->frac_position;*/
	if (num->len_frac)
	{
		sign_digit = ft_base_len(num->fractional[num->frac_position], 10);
		sign_digit += num->frac_position * 8;
	}
...
//	ft_putnbr(sign_digit);
	...
	sp->format.len.zero = 0;
	if (num->len_frac)
		ft_add_lim_precision_num_to_buf(num, sp, sign_digit);
	ft_add_zero_to_buf(sp);
}
